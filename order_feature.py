...
def get_order_feature(data_start,data_end,saveFileName,
    ...
    order_features_all = pd.DataFrame()
    order_features_all['uid'] =  user_data['uid']
    order_features_all = pd.merge(order_features_all,order_features,how='left')  
    print('saving...')
    order_features_all.to_csv(dest_dir+saveFileName,index = False)
...
data_dir = 'data/'
dest_dir = 'features/' 
get_order_feature(data_start,data_end,'order_features.csv')
